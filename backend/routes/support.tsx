...
async function safeGet(query, params = [], fallbackQuery = null, fallbackParams = params) {
  try {
    return await dbOperations.get(query, params);
  } catch (error) {
    if (!fallbackQuery) throw error;
    return await dbOperations.get(fallbackQuery, fallbackParams);
  }
}
...
router.get('/dashboard', async (req, res) => {
  try {
    ...
    const responseTimeResult = await safeGet(
      `SELECT AVG(
      ...
      [],
      `SELECT AVG(response_minutes) as avg_response_minutes
       FROM (
         SELECT (
           julianday(MIN(CASE WHEN m.sender_type = 'support' THEN m.timestamp END)) -
           julianday(MIN(CASE WHEN m.sender_type != 'support' THEN m.timestamp END))
         ) * 24 * 60 as response_minutes
         FROM messages m
         GROUP BY m.conversation_id
         HAVING MIN(CASE WHEN m.sender_type = 'support' THEN m.timestamp END) IS NOT NULL
            AND MIN(CASE WHEN m.sender_type != 'support' THEN m.timestamp END) IS NOT NULL
       )`
    );
