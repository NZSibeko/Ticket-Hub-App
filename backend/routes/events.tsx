// backend/routes/events.tsx - ES Module Version
...
if (!process.env.JWT_SECRET) {
  console.warn(
    "[AUTH] JWT_SECRET not set. Using development fallback secret. Set backend/.env JWT_SECRET for production.",
  );
}
...
const parseEventData = (event) => {
  ...
  try {
    if (event.ticket_types) {
      if (
        typeof event.ticket_types === "string" &&
        event.ticket_types.trim() !== ""
      ) {
    ...
    console.error(
      `Warning: Failed to parse ticket_types for event ID ${event.event_id || event.id}`,
      parseError,
    );
    ticketTypes = [];
  }
...
const fetchPublicEvents = async (dbOperations, limit = 50) => {
  // Use schema-safe columns only; older/newer DB variants differ on archived/start_date.
  try {
    return await dbOperations.all(
      `
        SELECT * FROM events
        WHERE LOWER(COALESCE(status, 'active')) IN ('active', 'validated')
    ...
    console.warn(
      "[EVENTS API] Primary public query failed, using fallback:",
...
const getPublicEvents = async (req, res) => {
  try {
    ...
    const events = await fetchPublicEvents(dbOperations, 20);
...
const getAllEventsHandler = async (req, res) => {
  try {
    ...
    console.log(
      `👤 User: ${userEmail}, Role: ${userRole}, Is Admin: ${isAdmin}`,
    );
    ...
    if (isAdmin) {
      ...
      query = `
        SELECT e.*,
               COALESCE(a.name, em.name, eo.name, ss.name, NULLIF(TRIM(COALESCE(c.first_name, '') || ' ' || COALESCE(c.last_name, '')), '')) as creator_name,
    ...
    const parsedEvents = events.map((event) => {
      ...
      try {
        ...
        console.error(
          `Error parsing ticket_types for event ${event.event_id}:`,
          error,
        );
        ticketTypes = [];
      }
    ...
    const counts = {
      ...
      pending: parsedEvents.filter(
        (e) => e.status === "PENDING" && e.archived !== 1,
      ).length,
      validated: parsedEvents.filter(
        (e) => e.status === "VALIDATED" && e.archived !== 1,
      ).length,
...
router.get("/public", async (req, res) => {
  try {
    ...
    const events = await fetchPublicEvents(dbOperations, 50);
...
router.get("/public/validated", async (req, res) => {
  try {
    ...
    const events = await fetchPublicEvents(dbOperations, 200);

    const parsedEvents = events.map(parseEventData);

    console.log(
      `✅ [EVENTS API] Found ${parsedEvents.length} validated events for public`,
    );
  ...
  try {
    ...
    if (!event_name || !location || !start_date || !max_attendees) {
      return res.status(400).json({
        success: false,
        error:
          "Missing required fields: name, location, start date, and max attendees",
      });
    }
    ...
    if (!eventId) {
      const row = await dbOperations.get(`SELECT last_insert_rowid() as id`);
      eventId = row?.id;
      console.log(`⚠ Using fallback event ID: ${eventId}`);
    }

    console.log(
      `✅ [EVENTS API] Created event ${eventId}: ${event_name} with status: ${initialStatus}`,
    );

    try {
      await dbOperations.run(
        `INSERT INTO event_creation_registry (
          registry_id, event_id, creator_user_id, creator_email, creator_role, created_from, approval_required, created_at
        ) VALUES (?, ?, ?, ?, ?, ?, ?, datetime('now'))`,
        [
          uuidv4(),
          eventId,
          user.userId || user.id || user.manager_id || user.admin_id || null,
          userEmail,
          userRole,
          "web",
      ...
      console.warn(
        "⚠️ [EVENTS API] Failed to write event creation registry:",
  ...
  try {
    ...
    try {
      ...
      if (validTicketTypes.length === 0) {
        ...
        try {
          if (
            existingEvent.ticket_types &&
            typeof existingEvent.ticket_types === "string"
          ) {
...
router.get("/search/validated", requireStaffAccess, async (req, res) => {
  try {
    ...
    console.log(
      `✅ [EVENTS API] Found ${parsedEvents.length} events matching: ${query}`,
    );
...
router.get("/pending/approvals", requireStaffAccess, async (req, res) => {
  try {
    ...
    const pendingEvents = await dbOperations.all(`
      SELECT e.*, 
             COALESCE(a.name, em.name, eo.name, ss.name, NULLIF(TRIM(COALESCE(c.first_name, '') || ' ' || COALESCE(c.last_name, '')), '')) as organizer_name,
    ...
    console.log(
      `✅ [EVENTS API] Found ${parsedEvents.length} pending events for approval`,
    );
...
router.get("/:id/approval-details", requireStaffAccess, async (req, res) => {
  try {
    const eventId = req.params.id;
    console.log(
      `📋 [EVENTS API] Getting approval details for event ${eventId}...`,
    );
    ...
    const event = await dbOperations.get(
      `
      SELECT e.*,
             COALESCE(a.name, em.name, eo.name, ss.name, NULLIF(TRIM(COALESCE(c.first_name, '') || ' ' || COALESCE(c.last_name, '')), '')) as organizer_full_name,
             COALESCE(a.email, em.email, eo.email, ss.email, c.email) as organizer_email,
    ...
    console.log(
      `✅ [EVENTS API] Retrieved approval details for event ${eventId}`,
    );
