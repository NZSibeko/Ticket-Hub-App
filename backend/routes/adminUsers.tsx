...
const getDbOperations = () => require('../database').dbOperations;
...
const requireAdminOrManager = (req, res, next) => {
    ...
    return res.status(403).json({ 
        success: false, 
        error: 'Access denied. Requires Admin or Manager role.',
        userRole: userRole,
        userRoles: user
    });
};
...
const generateUserLastActive = (userIndex, userRole) => {
    ...
    if (userRole === 'Manager') roleAdjustment = -15; // Managers somewhat recent
...
const fetchAllUsers = async () => {
    try {
        const dbOperations = getDbOperations();
        if (!dbOperations) {
            return [];
        }
        ...
        const queryWithFallback = async (primaryQuery, fallbackQuery) => {
            try {
                return await dbOperations.all(primaryQuery);
            } catch (err) {
                console.warn('Primary query failed, using fallback:', err.message);
                return await dbOperations.all(fallbackQuery);
            }
        };

        const customers = await queryWithFallback(
            `
            ...
            `
            SELECT customer_id as id, first_name, last_name, email, 'Customer' as role,
                   'active' as status, created_at as joined, NULL as phone_number, 'South Africa' as country
            FROM customers
            `
        );

        const managers = await queryWithFallback(
            `
            SELECT manager_id as id, name, email, 'Manager' as role,
                   status, created_at as joined, phone as phone_number, 'South Africa' as country
            FROM event_managers
            `,
            `
        ...
        allUsers.sort((a, b) => new Date(a.joined || 0).getTime() - new Date(b.joined || 0).getTime());
...
router.get('/dashboard', requireAdminOrManager, async (req, res) => {
    try {
        ...
        const roleDistributionPerc = Object.fromEntries(
            Object.entries(roleDistribution).map(([role, count]) => [role, Math.round((Number(count) / totalUsers) * 100)])
        );
        ...
        const recentActivity = [
            ...
            {
                type: 'role_changed',
                user: userList.find(u => u.role === 'Manager')?.name || 'Manager',
                time: generateRealisticTime(2, 0, 0), // 2 days ago
                status: 'warning',
                details: 'Role upgraded from Customer to Manager'
            },
            {
        ...
        res.json({
            ...
            warning: 'Fallback dashboard data returned'
        });
    }
});
...
router.post('/create', requireAdminOrManager, async (req, res) => {
    ...
    try {
        ...
        if (role === 'admin') {
            ...
            insertParams = [userId, fullName, email, hashedPassword, phone || null, 'active', 'manager', now, now];
        }
        else if (role === 'omni_support_consultant' || role === 'support') {
            tableName = 'support_staff';
            insertQuery = `INSERT INTO support_staff (support_id, name, email, password, phone, department, role, status, last_login, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`;
            insertParams = [userId, fullName, email, hashedPassword, phone || null, 'omni_support', 'omni_support_consultant', 'active', now, now];
        }
        else if (role === 'event_support_consultant') {
            tableName = 'support_staff';
            insertQuery = `INSERT INTO support_staff (support_id, name, email, password, phone, department, role, status, last_login, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`;
            insertParams = [userId, fullName, email, hashedPassword, phone || null, 'event_operations', 'event_support_consultant', 'active', now, now];
        }
        else if (role === 'event_organizer') {
            tableName = 'event_organizers';
            insertQuery = `INSERT INTO event_organizers (organizer_id, name, email, password, phone, company, role, status, last_login, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`;
            insertParams = [userId, fullName, email, hashedPassword, phone || null, null, 'event_organizer', 'active', now, now];
        }
        else {
            tableName = 'customers';
        ...
        const roleDisplay = role === 'customer' ? 'Customer' : 
                            (role === 'manager' || role === 'event_manager') ? 'Manager' :
                            (role === 'omni_support_consultant' || role === 'support') ? 'Omni Support Consultant' :
                            role === 'event_support_consultant' ? 'Event Support Consultant' :
...
router.put('/:id/suspend', requireAdminOrManager, async (req, res) => {
    ...
    try {
        ...
        const tables = [
            ...
            { name: 'event_managers', idColumn: 'manager_id', displayName: 'Manager' },
...
router.get('/:id', requireAdminOrManager, async (req, res) => {
    ...
    try {
        ...
        const tables = [
            ...
            { name: 'event_managers', idColumn: 'manager_id', role: 'Manager' },
            { name: 'event_organizers', idColumn: 'organizer_id', role: 'Event Organizer' },
            { name: 'support_staff', idColumn: 'support_id', role: 'Support Consultant' },
        ...
        if (userData.last_login) {
            ...
            const now = new Date();
            const diffHours = Math.floor((now.getTime() - lastLogin.getTime()) / 3600000);
            
            if (diffHours < 1) {
                const diffMins = Math.floor((now.getTime() - lastLogin.getTime()) / 60000);
...
router.put('/:id/change-role', requireAdminOrManager, async (req, res) => {
    ...
    if (!newRole || !['customer', 'manager', 'event_manager', 'event_organizer', 'omni_support_consultant', 'event_support_consultant', 'admin'].includes(newRole)) {
        return res.status(400).json({ success: false, error: 'Invalid role. Must be customer, manager, event_organizer, omni_support_consultant, event_support_consultant, or admin' });
    }
    ...
    try {
        ...
        const currentRole = currentTable.name === 'customers' ? 'customer' : 
                           currentTable.name === 'event_managers' ? 'manager' :
                           currentTable.name === 'event_organizers' ? 'event_organizer' :
                           currentTable.name === 'support_staff' ? (currentUser.role || 'omni_support_consultant') : 'admin';
        ...
        switch(newRole) {
            ...
            case 'event_organizer':
                targetTable = 'event_organizers';
                targetIdColumn = 'organizer_id';
                break;
            case 'omni_support_consultant':
            case 'event_support_consultant':
                targetTable = 'support_staff';
                targetIdColumn = 'support_id';
                break;
            case 'admin':
                targetTable = 'admins';
        ...
        try {
            ...
            } else if (newRole === 'manager' || newRole === 'event_manager') {
            ...
            } else if (newRole === 'event_organizer') {
                const fullName = currentUser.name || `${currentUser.first_name || ''} ${currentUser.last_name || ''}`.trim();
                await dbOperations.run(
                    `INSERT INTO event_organizers (organizer_id, name, email, password, phone, company, status, role, last_login, created_at)
                     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
                    [
                        userId,
                        fullName,
            ...
            } else if (newRole === 'omni_support_consultant' || newRole === 'event_support_consultant') {
                const fullName = currentUser.name || `${currentUser.first_name || ''} ${currentUser.last_name || ''}`.trim();
                const targetDepartment = newRole === 'event_support_consultant' ? 'event_operations' : 'omni_support';
                await dbOperations.run(
                    `INSERT INTO support_staff (support_id, name, email, password, phone, department, status, role, last_login, created_at)
                     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
                    [
                        userId,
                        fullName,
            ...
            const roleDisplay = newRole === 'customer' ? 'Customer' : 
                               (newRole === 'manager' || newRole === 'event_manager') ? 'Manager' :
                               (newRole === 'omni_support_consultant' || newRole === 'support') ? 'Omni Support Consultant' :
                               newRole === 'event_support_consultant' ? 'Event Support Consultant' :
                               newRole === 'event_organizer' ? 'Event Organizer' : 'Admin';
            ...
            if (newRole === 'customer') {
                ...
                updatedUser = await dbOperations.get(
                    `SELECT * FROM event_organizers WHERE organizer_id = ?`,
                    [userId]
                );
            } else if (newRole === 'omni_support_consultant' || newRole === 'event_support_consultant') {
                updatedUser = await dbOperations.get(
                    `SELECT * FROM support_staff WHERE support_id = ?`,
                    [userId]
                );
            } else {
                updatedUser = await dbOperations.get(
...
router.delete('/:id/delete', requireAdminOrManager, async (req, res) => {
    ...
    try {
        ...
        try {
            ...
            try {
                // If the user is a manager, archive their events
