    ...
    try {
      ...
      for (const tableInfo of tablesToClean) {
        try {
          ...
          const countQuery = `SELECT COUNT(*) as count FROM ${tableInfo.table} WHERE ${tableInfo.dateField} < ?`;
          if (tableInfo.where) {
          ...
          if (countToDelete > 0) {
            if (!dryRun) {
              ...
              const deleteQuery = `DELETE FROM ${tableInfo.table} WHERE ${tableInfo.dateField} < ?`;
