...
import axios from 'axios'; // Added import for used library
import ScreenContainer from '../components/ScreenContainer';
import { useAuth } from '../context/AuthContext';

// --- ASSUMED INTERFACES ---
// Interface for IUser, inferred from usage in fetchTickets (user.customer_id)
interface IUser {
  customer_id: string;
...
interface Ticket {
  ...
  location: string;
  ticket_type: 'early_bird' | 'general' | 'family_group' | 'vip' | 'vvip' | 'premium';
  price: number;
  currency: string;
  ticket_status: 'ACTIVE' | 'VALIDATED' | 'CANCELLED' | 'REFUNDED';
...
// Props for MyTicketsScreen component (assuming standard navigation prop structure)
interface MyTicketsScreenProps {
  navigation: any; // Replace with specific navigation type if available
}

// Context type for useAuth hook
...
const getTicketTypeColor = (type: Ticket['ticket_type']): string => {
  const colors: Record<Ticket['ticket_type'], string> = {
    early_bird: '#10b981',
    general: '#6366f1',
    family_group: '#8b5cf6',
    vip: '#f59e0b',
...
const getStatusColor = (status: Ticket['ticket_status']): string => {
  switch (status) {
    case 'ACTIVE':
      return '#10b981';
    case 'VALIDATED':
...
const getStatusIcon = (status: Ticket['ticket_status']): string => {
  switch (status) {
    case 'ACTIVE':
...
const TicketCard: FC<TicketCardProps> = React.memo(({ item, onShowQR, index }) => {
  const [imageError, setImageError] = useState<boolean>(false);
  const [imageLoaded, setImageLoaded] = useState<boolean>(false);
  const isPast: boolean = new Date(item.event_date) < new Date() || item.ticket_status === 'CANCELLED';
  const statusColor: string = getStatusColor(item.ticket_status);
  const ticketTypeColor: string = getTicketTypeColor(item.ticket_type);
  const soldPercentage: number = (item.current_attendees / item.max_attendees) * 100;
...
const DetailItem: FC<DetailItemProps> = ({ icon, label, value }) => (
...
const MyTicketsScreen: FC<MyTicketsScreenProps> = ({ navigation }) => {
  ...
  const openQRModal = useCallback((ticket: Ticket): void => {
    setSelectedTicket(ticket);
    setShowQRModal(true);
  }, []);

  const closeQRModal = useCallback((): void => {
  ...
  }, [currentIndex, filteredTickets.length, scrollToIndex]);
  ...
  const onMomentumScrollEnd = useCallback((event: any): void => {
  ...
  const shareTicket = async (platform: string = 'more'): Promise<void> => {
  ...
  const renderTicketItem = useCallback(({ item, index }: { item: Ticket, index: number }) => (
  ...
  const keyExtractor = useCallback((item: Ticket) => item.ticket_id, []);

  const getItemLayout = (data: Ticket[], index: number) => ({
    length: 320 + 16,
    offset: (320 + 16) * index,
    index,
  });
  ...
  const ShareButton: FC<{ platform: string, icon: string, color: string, label: string }> = ({ platform, icon, color, label }) => (
    <TouchableOpacity 
    ...
    return (
      <ScreenContainer>
        <View style={styles.notLoggedInContainer}>
          ...
          <Text style={styles.notLoggedInTitle}>Log in to see your tickets</Text>
          <Text style={styles.notLoggedInText}>
            Please sign in to view your purchased tickets and event history
          </Text>
